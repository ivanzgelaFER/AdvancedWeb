    ...
    async getHtml() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                let recipes = allRecipes.filter(x => Math.random()>0.75).reverse();  // bcs the last items tend to have valid image...

                return `<h1>Recipes (${recipes.length})</h1><div class="container-fluid p-2 d-flex flex-wrap">`
                    + recipes.map(rcp => `
